...
function avatarProxy(url: string, revision?: number) {
  try {
    return `${wallifyImageUrl(new URL(url).pathname)}&v=${revision ?? 0}`;
  } catch {
    return url;
  }
}
...
export default function SettingsScreen() {
  const { isLoading, saveProfile } = useWallifyProfile();
  const { session, isLoading: isSessionLoading, saveSession, clearSession } = useWallifySession();
  const refreshSessionProfile = trpc.wallify.sessionProfile.useQuery({ token: session?.token ?? "000000000000000000000000000000000000000000000000" }, { enabled: false });
  const version = trpc.wallify.version.useQuery(undefined, { enabled: false });
  ...
  const handleRefresh = async () => {
    try {
      if (session) {
        ...
        if (refreshed.data) {
          const nextProfile = refreshed.data.profile as LinkedWallifyProfile;
          await saveSession({ token: session.token, expiresAt: refreshed.data.tokenExpiresAt ?? session.expiresAt, profile: nextProfile });
  ...
  const handleSignOut = () => {
    Alert.alert("退出登录", "这会使当前 Wallify token 失效并清除本设备保存的登录状态。", [
      { text: "取消", style: "cancel" },
      { text: "退出", style: "destructive", onPress: () => { if (session) void logout.mutateAsync({ token: session.token }).catch(() => undefined); void clearSession(); } },
    ]);
  };

  const displayedProfile = session?.profile ?? null;
  ...
  const checkForUpdates = async () => {
    try {
      ...
      if (!latestVersion) throw new Error("版本服务未返回最新版本");
      if (compareVersions(installedVersion, latestVersion) < 0) {
        Alert.alert("发现新版本", `最新版本为 v${latestVersion}，可从 Wallify 官网下载安装。`, [{ text: "知道了" }]);
      } else {
        Alert.alert("已是最新版本", `当前安装包 v${installedVersion} 已是 AppAPI 提供的最新版本。`);
      }
    } catch (error) {
      const issue = getWallifyServiceIssue(error);
      Alert.alert(issue?.title ?? "检查更新失败", issue?.description ?? (error instanceof Error ? error.message : "请稍后再试。"));
    }
  };
    ...
    <ScreenContainer>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {loading ? <View style={styles.loading}><ActivityIndicator color="#7D9EFF" /></View> : displayedProfile ? (
          <ProfileCard profile={displayedProfile} isRefreshing={refreshSessionProfile.isFetching} onRefresh={() => void handleRefresh()} onEdit={() => router.push("/profile-edit" as never)} onUpload={() => router.push("/upload-wallpaper" as never)} onSignOut={handleSignOut} />
        ) : <GuestCard onLogin={() => router.push("/login" as never)} />}
        {!displayedProfile ? null : <View style={styles.note}><IconSymbol name="lock.fill" size={16} color="#FFB86B" /><Text style={styles.noteText}>AppAPI 会话令牌保存在设备安全存储中，密码不会保存；资料和账户操作通过 Wallify 官方 API 完成。</Text></View>}
