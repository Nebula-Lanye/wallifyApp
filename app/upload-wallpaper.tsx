...
export default function UploadScreen() {
  ...
  const appApiUploadUnavailable = true;
  ...
  const submit = async () => {
    ...
    if (appApiUploadUnavailable) {
      Alert.alert("暂不支持发布", "当前 AppAPI v1.4 尚未提供壁纸上传 action。beta 版暂时保留选图和表单预览，待官方接口开放后启用发布。", [{ text: "知道了" }]);
      return;
    }
  ...
  return (
    <ScreenContainer edges={["top", "bottom", "left", "right"]}>
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.lead}>从设备相册准备壁纸，在 Wallify 原生发布。</Text>
        {appApiUploadUnavailable ? <View style={styles.apiNotice}><IconSymbol name="info.circle" size={18} color="#FFCB6B" /><Text style={styles.apiNoticeText}>AppAPI v1.4 暂未定义壁纸上传 action。当前可以使用系统选择器预览文件，但不能提交到服务器。</Text></View> : null}
        ...
        <Pressable onPress={() => void submit()} disabled={upload.isPending || appApiUploadUnavailable} style={({ pressed }) => [styles.primaryButton, (pressed || upload.isPending || appApiUploadUnavailable) && styles.pressed]}>{upload.isPending ? <ActivityIndicator color="#FFFFFF" /> : <><IconSymbol name="square.and.arrow.up" size={18} color="#FFFFFF" /><Text style={styles.primaryText}>{appApiUploadUnavailable ? "等待上传接口开放" : "发布到 Wallify"}</Text></>}</Pressable>
      </ScrollView>
